...
const matLine=new T.MeshStandardMaterial({color:'#e8dcc3',roughness:.6,side:T.DoubleSide});
 ...
 let dbg:{s:number;yaw:number;chunks:number[];npcs:{s:number;lane:number;spd:number;off:number}[];spawn:{x:number;z:number;yaw:number};overtakes:number;cross:number;lastDrift:number;glbCars:number;carCars:number}={s:0,yaw:0,chunks:[],npcs:[],spawn:{x:0,z:0,yaw:YawOf(0)},overtakes:0,cross:0,lastDrift:-9e9,glbCars:0,carCars:0};
 ...
 function update(dt:number,px:number,pz:number,drifting:boolean,clock:number){if(!group.visible)return null;frame++;const p=project(px,pz,lastS);lastS=p.s;const ns=p.s;extendAhead(ns+AHEAD+PAD);dropFront(Math.max(0,ns-BEHIND-PAD));syncChunks(ns);
  ...
  dbg.glbCars=glbCars;dbg.carCars=carCars;
