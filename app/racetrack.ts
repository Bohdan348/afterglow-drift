...
import {preloadNpcModels,npcCarModel,NPC_CARS,npcCarColors} from './npc';
...
interface Npc{s:number;lane:number;off:number;spd:number;col:number;lastOv:number;seed:number;u:string;grp:T.Group;wheels:T.Object3D[];glbMats:T.Material[]}
...
const matLine=new T.MeshStandardMaterial({color:'#e8dcc3',roughness:.6,side:T.DoubleSide});
 ...
 const matBarrier=new T.MeshStandardMaterial({color:'#cc2222',roughness:.65,vertexColors:true,side:T.DoubleSide});
 ...
 let dbg:{s:number;yaw:number;chunks:number[];npcs:{s:number;lane:number;spd:number;off:number}[];spawn:{x:number;z:number;yaw:number};overtakes:number;cross:number;lastDrift:number;barrierCount:number;glbCars:number;carCars:number}={s:0,yaw:0,chunks:[],npcs:[],spawn:{x:0,z:0,yaw:YawOf(0)},overtakes:0,cross:0,lastDrift:-9e9,barrierCount:0,glbCars:0,carCars:0};
 ...
 function buildChunk(k:number){const s0=k*CHUNK,s1=s0+CHUNK;if(s1<=sBase)return;const i0=Math.max(0,Math.ceil((s0-sBase)/RES)),i1=Math.min(SX.length-2,Math.floor((s1-sBase)/RES)+1);if(i1<=i0)return;const g=new T.Group();
  ...
  for(let s=s0+30;s<s1-30;s+=70){if(li>=20)break;const o=12.5,p=posAt(s),yaw=YawOf(s),ry=Math.cos(yaw),rn=-Math.sin(yaw),x=p[0]+ry*o,z=p[1]+rn*o,y=HOf(s)+o*BankOf(s)+Terr(o);dummy.position.set(x,y,z);dummy.rotation.set(0,yaw+Math.PI/2,0);dummy.scale.setScalar(1);dummy.updateMatrix();poles.setMatrixAt(li,dummy.matrix);dummy.position.set(x,y+7.6,z);dummy.updateMatrix();heads.setMatrixAt(li,dummy.matrix);li++}
   ...
   for(const side of[-1,1]){const o=side*LIMIT,H=1.15;const pos:number[]=[],col:number[]=[],idx:number[]=[];for(let i=i0;i<=i1;i++){const s=sBase+i*RES,p=posAt(s),yaw=YawOf(s),ry=Math.cos(yaw),rn=-Math.sin(yaw),yB=HOf(s)+o*BankOf(s)+.07,yT=yB+H;pos.push(p[0]+ry*o,yB,p[1]+rn*o);pos.push(p[0]+ry*o,yT,p[1]+rn*o);const c=((i>>2)&1)===1;col.push(c?1:0,c?0:0,c?0:1,c?1:0,c?0:0,c?0:1)}const cw=i1-i0+1;for(let i=0;i<cw-1;i++){const a=i*2,b=a+2,c2=a+1,d=c2+2;idx.push(a,b,c2,c2,b,d)}const barGeo=new T.BufferGeometry();barGeo.setAttribute('position',new T.Float32BufferAttribute(pos,3));barGeo.setAttribute('color',new T.Float32BufferAttribute(col,3));barGeo.setIndex(idx);barGeo.computeVertexNormals();const m=new T.Mesh(barGeo,matBarrier);m.castShadow=false;m.receiveShadow=true;g.add(m)}
 ...
 const cabGeo=(()=>{const c=new T.BoxGeometry(1.6,.5,2.1);c.translate(0,1.14,-.1);return c})();
 ...
 const fallbackMat=new T.MeshStandardMaterial({color:'#ffffff',roughness:.55,metalness:.3});
 function makeTrackVeh(u:string,color:string){const model=npcCarModel(u);if(model){const g=model.group.clone(true);const glbMats:T.Material[]=[];g.traverse(o=>{if(!(o instanceof T.Mesh))return;o.castShadow=false;o.receiveShadow=false;const mats=Array.isArray(o.material)?o.material:[o.material];const nm:T.Material[]=[];for(const mm of mats){const m0=mm as T.MeshStandardMaterial;if(m0.isMeshStandardMaterial&&m0.name==='MatBody'){const c=new T.MeshStandardMaterial({color,roughness:.55,metalness:.25,...(m0.map?{map:m0.map}:{})});glbMats.push(c);nm.push(c)}else nm.push(mm)}o.material=nm.length===1?nm[0]:nm});const wheels:T.Object3D[]=[];g.children.forEach(c=>{if(c instanceof T.Group){c.userData.glb=true;wheels.push(c)}});g.position.y=.03;return{grp:g,wheels,glbMats}}const g=new T.Group();const mB=new T.Mesh(carGeo,fallbackMat);g.add(mB);const mC=new T.Mesh(cabGeo,fallbackMat);g.add(mC);const wheels:T.Object3D[]=[];for(const sx of[-1,1])for(const sz of[-1,1]){const w=new T.Mesh(wheelGeo,wheelMat);w.position.set(sx*.82,.4,sz*1.45);g.add(w);wheels.push(w)}return{grp:g,wheels,glbMats:[] as T.Material[]}}
 function dropNpc(n:Npc){group.remove(n.grp);for(const m of n.glbMats)m.dispose()}
 const laneC=(l:number)=>(l-2)*LANE_W;
 function spawnNpc(pS:number,fresh:boolean){if(npcs.length>=NPC_MAX)return;const s=fresh?pS+(90+Math.random()*500):pS+(600+Math.random()*1600);const lane=Math.random()<.75?2+((Math.random()*3)|0):((Math.random()*2)|0);const off=laneC(lane)+(Math.random()-.5)*1.6,spd=SPD_MIN+Math.random()*(SPD_MAX-SPD_MIN),col=(Math.random()*npcCarColors.length)|0,u=NPC_CARS[(Math.random()*NPC_CARS.length)|0],mv=makeTrackVeh(u,npcCarColors[col]);group.add(mv.grp);npcs.push({s,lane,off,spd,col,lastOv:-9,seed:Math.random()*1e9,u,grp:mv.grp,wheels:mv.wheels,glbMats:mv.glbMats})}
 const qW=new T.Quaternion(),eY=new T.Euler();
 function update(dt:number,px:number,pz:number,drifting:boolean,clock:number){if(!group.visible)return null;frame++;const p=project(px,pz,lastS);lastS=p.s;const ns=p.s;extendAhead(ns+AHEAD+PAD);dropFront(Math.max(0,ns-BEHIND-PAD));syncChunks(ns);
  lastTop-=dt;if(lastTop<=0){lastTop=.5;while(npcs.length<NPC_MIN)spawnNpc(ns,true);if(npcs.filter(w=>w.s>ns&&w.s<ns+700).length<3)spawnNpc(ns,false);for(const n of npcs)if(!n.glbMats.length&&npcCarModel(n.u)){const mv=makeTrackVeh(n.u,npcCarColors[n.col]);group.remove(n.grp);group.add(mv.grp);n.grp=mv.grp;n.wheels=mv.wheels;n.glbMats=mv.glbMats}}
  ...
  for(const n of npcs)if(n.s<ns-900||n.s>ns+3400)dropNpc(n);
  ...
  prevP=ns;
  let glbCars=0,carCars=0;
  for(const n of npcs){const pp=posAt(n.s),yaw=YawOf(n.s),ry=Math.cos(yaw),rn=-Math.sin(yaw),fy=-Math.sin(yaw),fz=-Math.cos(yaw),x=pp[0]+ry*n.off,z=pp[1]+rn*n.off,y=HOf(n.s)+n.off*BankOf(n.s)+.03;if(n.glbMats.length)glbCars++;carCars++;eY.set(0,yaw+Math.sin(n.s*.06+n.seed)*.012,0);qW.setFromEuler(eY);n.grp.position.set(x,y,z);n.grp.quaternion.copy(qW);n.grp.scale.setScalar(1);const spin=n.spd*dt/.4;for(const w of n.wheels)if(w.userData.glb)w.rotation.x+=spin;else w.rotation.z+=spin;n.grp.visible=true}
  dbg.glbCars=glbCars;dbg.carCars=carCars;dbg.barrierCount=chunks.size*2;
 ...
 function reset(){for(const k of[...chunks.keys()]){const c=chunks.get(k)!;group.remove(c);c.traverse(o=>{if(o instanceof T.Mesh&&!(o.userData as {shared?:boolean}).shared)o.geometry.dispose()});chunks.delete(k)}SX.length=0;SZ.length=0;sBase=0;extendAhead(AHEAD+PAD);for(const n of npcs)dropNpc(n);npcs=[];lastTop=0;prevP=NaN;const p0=posAt(0);dbg.spawn={x:Math.round(p0[0]*10)/10,z:Math.round(p0[1]*10)/10,yaw:Math.round(YawOf(0)*100)/100};for(let i=0;i<NPC_MIN;i++)spawnNpc(0,true);dbg.glbCars=0;dbg.carCars=npcs.length;dbg.barrierCount=0}
 function destroy(){for(const n of npcs)dropNpc(n);npcs=[];for(const k of[...chunks.keys()]){const c=chunks.get(k)!;group.remove(c);c.traverse(o=>{if(o instanceof T.Mesh&&!(o.userData as {shared?:boolean}).shared)o.geometry.dispose()});chunks.delete(k)}matAsphalt.dispose();matGrass.dispose();matLine.dispose();matCurb.dispose();matBarrier.dispose();farGround.geometry.dispose();(farGround.material as T.Material).dispose();grain.dispose();treeGeo.dispose();treeMat.dispose();lampPoleGeo.dispose();lampHeadGeo.dispose();poleM.dispose();headM.dispose();carGeo.dispose();cabGeo.dispose();wheelGeo.dispose();wheelMat.dispose();fallbackMat.dispose();scene.remove(group);(window as unknown as {__raceway?:unknown}).__raceway=null}
