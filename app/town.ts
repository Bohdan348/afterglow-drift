...
export function makeHeight(data:TownData){const e=data.elev;if(!e)return()=>0;const{o,res,nx,nz,data:_}=e;return(x:number,z:number)=>{const fx=(x-o[0])/res,fz=(z-o[1])/res;let ix=Math.floor(fx),iz=Math.floor(fz);const tx=fx-ix,tz=fz-iz;ix=Math.max(0,Math.min(nx-2,ix));iz=Math.max(0,Math.min(nz-2,iz));const i=iz*nx+ix,v00=_[i],v10=_[i+1],v01=_[i+nx],v11=_[i+nx+1];if(tx>=tz)return(1-tx)*v00+(tx-tz)*v10+tz*v11;return(1-tz)*v00+tx*v11+(tz-tx)*v01}}
...
const land=mat('#788564'),grass=mat('#6a8057'),asphalt=mat('#3b4244'),path=mat('#a6a28b'),roofMat=mat('#606c70'),water= new T.MeshStandardMaterial({color:'#457e87',roughness:.27,metalness:.6});
  ...
  if(data.elev){
   ...
   const ground=new T.Mesh(new T.PlaneGeometry(gwx,gwz,nx-1,nz-1),land);ground.rotation.x=-Math.PI/2;ground.position.set(o[0]+gwx/2,0,o[1]+gwz/2);const gv=ground.geometry.getAttribute('position')as T.BufferAttribute;for(let i=0;i<gv.count;i++){const wx=gv.getX(i)+ground.position.x,wz=gv.getZ(i)+ground.position.z;gv.setY(i,height(wx,wz))}gv.needsUpdate=true;ground.geometry.computeVertexNormals();ground.receiveShadow=true;ground.castShadow=false;ground.matrixAutoUpdate=false;ground.updateMatrix();group.add(ground);
   const table=new T.Mesh(new T.PlaneGeometry(20000,20000),land);table.rotation.x=-Math.PI/2;table.position.set(ground.position.x,-1.6,ground.position.z);table.receiveShadow=true;table.castShadow=false;group.add(table);
  }else{
  ...
  for(const a of data.areas){if(a.kind==='water')flat(a.p,-.08,water);else if(['grass','forest','wood','park','meadow','recreation_ground','village_green','orchard','vineyard','farmland'].includes(a.kind))flat(a.p,.015,grass);else if(['pitch','parking','pedestrian'].includes(a.kind))flat(a.p,.018,path)}
 const roadSegments:{a:Point,b:Point,width:number}[]=[];
 for(const road of data.roads){const drivable=['residential','service','unclassified'].includes(road.kind)||['primary','secondary','tertiary','living_street'].includes(road.kind),w=road.width;for(let i=1;i<road.p.length;i++){const a=road.p[i-1],b=road.p[i],dx=b[0]-a[0],dz=b[1]-a[1],length=Math.hypot(dx,dz);if(length<.05)continue;const g=new T.PlaneGeometry(w,length+.35);g.rotateX(-Math.PI/2);g.rotateY(Math.atan2(dx,dz));g.translate((a[0]+b[0])/2,.015,(a[1]+b[1])/2);const uv=g.getAttribute('uv'),pos=g.getAttribute('position');for(let k=0;k<uv.count;k++)uv.setXY(k,pos.getX(k)/8,pos.getZ(k)/8);drape(g,.02);add(g,drivable?asphalt:path);if(drivable){roadSegments.push({a,b,width:w});const disc=new T.CircleGeometry(w/2,10);disc.rotateX(-Math.PI/2);disc.translate(a[0],.016,a[1]);drape(disc,.025);add(disc,asphalt)}}}
