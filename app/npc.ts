...
function buildP(roads:Point[][],kinds:string[],widths:number[],areas:{p:Point[];kind:string}[]):G{const g:G={nodes:[],adj:[],edges:[]};const npos=new Map<number,GNode>();const grid=new Map<string,number[]>();const seen=new Set<string>();const link=new Set<string>();
 ...
 return g}
interface Ped{slot:number;a:number;b:number;ei:number;t:number;x:number;z:number;yaw:number;spyaw:number;lat:number;speed:number;cap:number;phase:number;color:number;noCross:number;seed:number;st:string;gx:number;gz:number}
interface Veh{type:NpcType;a:number;b:number;ei:number;t:number;x:number;z:number;yaw:number;spyaw:number;lat:number;touch:number;yawOff:number;speed:number;cap:number;drift:boolean;drT:number;drDur:number;nextDrift:number;life:number;seed:number;yield:boolean;st:string;u:string;grp:T.Group;wheels:T.Object3D[];bodyMat:T.MeshStandardMaterial|null;glbMats:T.Material[];rad:number;hitT:number}
...
update(dt,px,pz,pvx:number=0,pvz:number=0,pr:number=0){lastTop-=dt;if(lastTop<=0){lastTop=.5;topup(px,pz)}
    const col={px:0,pz:0,vx:0,vz:0};
    const alive:Veh[]=[];
    for(const v of vehs){v.life+=dt;const dx=px-v.x,dz=pz-v.z;const dist=Math.hypot(dx,dz);let eff=v.cap;v.yield=false;
     ...
     if(pr>0){const gdx=px-v.grp.position.x,gdz=pz-v.grp.position.z;const gdist=Math.hypot(gdx,gdz);if(gdist<v.rad+pr&&gdist>.01){const nx=gdx/gdist,nz=gdz/gdist;const pen=Math.min(v.rad+pr-gdist,1.2);col.px+=nx*pen;col.pz+=nz*pen;const lx=-diz,lz=dix;v.lat+=(nx*lx+nz*lz)*pen;const vvx=dix*v.speed,vvz=diz*v.speed;const rel=(pvx-vvx)*nx+(pvz-vvz)*nz;if(rel<0){const b=v.type==='moto'?.6:1,e=.35;const vpn=pvx*nx+pvz*nz,vvn=vvx*nx+vvz*nz;const vpn2=((1-b)*vpn+(1+e)*b*vvn)/(1+b),vvn2=((1+e)*vpn+(b-1)*vvn)/(1+b);col.vx+=nx*(vpn2-vpn);col.vz+=nz*(vpn2-vpn);const fin=dix*nx+diz*nz;v.speed=Math.max(0,v.speed+(vvn2-vvn)*fin);v.hitT=.45}}}
     if(dx*dix+dz*diz>0&&dist<8&&v.speed>1){eff=Math.min(eff,Math.max(1,(dist/8)*v.cap));v.yield=true;v.st='yield'}
     for(const o of vehs){if(o===v||!o.grp.parent)continue;const od=Math.hypot(o.x-v.x,o.z-v.z);if((o.x-v.x)*dix+(o.z-v.z)*diz>0&&od<7){eff=Math.min(eff,(od/7)*v.cap)}const ogx=o.grp.position.x-v.grp.position.x,ogz=o.grp.position.z-v.grp.position.z,odg=Math.hypot(ogx,ogz);if(odg<v.rad+o.rad&&odg>.01){eff=Math.min(eff,Math.max(1,(odg/(v.rad+o.rad))*v.cap));const lt=ogx*diz-ogz*dix;const sep=Math.min((v.rad+o.rad-odg)*.8,1.4)*Math.min(1,dt*8);const s=Math.abs(lt)>.01?Math.sign(lt):(o.seed>v.seed?1:-1);v.lat+=s*sep;o.lat-=s*sep;if(Math.abs(lt)<1.5){const vl=v.rad*.5+odg*.2,ov=o.rad*.5+odg*.2;if(Math.abs(v.lat)<vl)v.lat=s*vl;if(Math.abs(o.lat)<ov)o.lat=-s*ov}v.touch=1;o.touch=1}else if(odg<v.rad+o.rad+2&&odg>.01){eff=Math.min(eff,Math.max(1,(odg/(v.rad+o.rad+2))*v.cap))}}
...
if(v.drift){v.drT+=dt;if(v.drT>=v.drDur){v.drift=false;v.st='ride'}else if(!v.touch&&!v.yield){const u=v.drT/v.drDur,s=Math.sin(Math.PI*u);v.lat=s*(2.0+(v.seed%6)/10);v.yawOff=s*(.18+(v.seed%13)/100)}v.yield=false}
    ...
    if(v.hitT>0){v.hitT-=dt;v.speed*=Math.exp(-dt*3)}
   ...
   vehs=alive;
   const aliveP:Ped[]=[];
for(const p of peds){p.noCross=Math.max(0,p.noCross-dt);p.lat*=Math.exp(-dt*4);p.speed+=(p.cap-p.speed)*Math.min(1,dt*2);
     step(pg,p,dt,(a,b)=>pedNext(a,b,p));
     p.x=clampx(p.x);p.z=clampz(p.z);
     if(pr>0){const pe0=pg.edges[p.ei];if(pe0){const qa0=pg.nodes[p.a],qb0=pg.nodes[p.b];let hx0=qb0.x-qa0.x,hz0=qb0.z-qa0.z;const hl0=Math.hypot(hx0,hz0)||1;hx0/=hl0;hz0/=hl0;const rdx=px-p.x,rdz=pz-p.z;const rp=Math.hypot(rdx,rdz);if(rp<3.4){const lx0=-hz0,lz0=hx0;const off=rdx*lx0+rdz*lz0;const need=2.2-Math.abs(off);if(need>0)p.lat-=Math.sign(off||1)*need*Math.min(1,dt*3);if(rdx*hx0+rdz*hz0>0)p.cap=Math.min(5,p.cap+dt*5)}}}
    ...
    const ex=pb.x-pa.x,ez=pb.z-pa.z;const el=Math.hypot(ex,ez)||1;const rxo=-ez/el,rzo=ex/el;
    dummy.position.set(p.x+rxo*p.lat,hy+.45+bob*.05,p.z+rzo*p.lat);dummy.rotation.set(0,p.yaw,0);dummy.scale.setScalar(1);pBody.setMatrixAt(p.slot,dummy.matrix);p.gx=p.x+rxo*p.lat;p.gz=p.z+rzo*p.lat;
...
debugObj.actors.length=0;for(const v of vehs)debugObj.actors.push({t:v.type,x:Math.round(v.x*10)/10,z:Math.round(v.z*10)/10,yaw:Math.round(v.yaw*100)/100,s:Math.round(v.speed*10)/10,dr:v.drift?1:0,yl:v.yield?1:0,st:v.st,gx:Math.round(v.grp.position.x*10)/10,gz:Math.round(v.grp.position.z*10)/10,life:Math.round(v.life*10)/10});for(const p of peds)debugObj.actors.push({t:'ped',x:Math.round(p.x*10)/10,z:Math.round(p.z*10)/10,yaw:Math.round(p.yaw*100)/100,s:Math.round(p.speed*10)/10,dr:0,yl:0,st:p.st,gx:Math.round(p.gx*10)/10,gz:Math.round(p.gz*10)/10});
    return col;
   }
 };
 return handle;
