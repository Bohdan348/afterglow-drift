...
export interface NpcHandle{respawn(cx:number,cz:number):void;despawn():void;destroy():void;update(dt:number,px:number,pz:number):void;debug():{counts:{ped:number;moto:number;car:number};actors:{t:NpcType;x:number;z:number;yaw:number;s:number;dr:number;yl:number;st:string}[];graph:{ve:number;pe:number}}}
...
export function createNpc(opts:{id:string;roads:{p:Point[];kind:string;width:number}[];areas:{p:Point[];kind:string}[];bounds:number[];height:(x:number,z:number)=>number;scene:T.Scene}):NpcHandle{
 ...
 const spawnMesh=(type:NpcType,color:string)=>{const g=new T.Group();const mat=new T.MeshStandardMaterial({color,roughness:.55,metalness:.25});const wheels:T.Mesh[]=[];
  ...
  scene.add(g);return{grp:g,wheels,mat}};
 ...
 const debugObj:ReturnType<NpcHandle['debug']>={counts:{ped:0,moto:0,car:0},actors:[],graph:{ve:vg.nodes.length,pe:pg.nodes.length}};
 ...
 const spawnVeh=(type:'moto'|'car',px:number,pz:number)=>{const ids=nearNodes(vg,px,pz,380);if(!ids.length)return;const lo=40,hi=type==='car'?300:260;const n=pickNode(ids,vg,px,pz,lo,hi);if(n<0)return;const out=vg.adj[n];if(!out.length)return;const carC=['#3f5f83','#7a3f3f','#3f7a5f','#8a7a3f','#5f3f7a','#3f7a7a'];const motoC=['#c9402f','#2f5fc9','#c9a02f','#2fc96a'];const m=spawnMesh(type,type==='car'?carC[(Math.random()*carC.length)|0]:motoC[(Math.random()*motoC.length)|0]);const b=out[(Math.random()*out.length)|0];if(b.n===n)return;const bck=vg.adj[b.n].find(en=>en.n===n);if(!bck){scene.remove(m.grp);m.mat.dispose();return}const ei=bck.ei;const kind=vg.edges[ei].kind;const km=KIND_MAX[kind]??12;const seed=empty(opts.id)+(type==='moto'?31:57)+vehs.length*131+7;const pr=rng(seed);const e=vg.edges[ei],pa=vg.nodes[n],pb=vg.nodes[b.n];const px0=pa.x+(pb.x-pa.x)*pr(),pz0=pa.z+(pb.z-pa.z)*pr(),hy=height(px0,pz0);const v:Veh={type,a:n,b:b.n,ei,t:pr()*.9,x:px0,z:pz0,yaw:0,spyaw:Math.atan2(-(pb.x-pa.x),-(pb.z-pa.z)),lat:0,yawOff:0,speed:km*.5,cap:Math.max(6,(km*(.75+pr()*.35))+(type==='moto'?3:0)),drift:false,drT:0,drDur:0,nextDrift:3+(pr()*7),life:0,seed,yield:false,st:'ride',grp:m.grp,wheels:m.wheels,bodyMat:m.mat};v.grp.position.set(px0,hy,pz0);v.grp.rotation.y=v.spyaw;vehs.push(v)};
 const killAll=()=>{for(const p of peds){freeSlots.push(p.slot);hideP(p.slot)}peds=[];for(const v of vehs){v.grp.parent?.remove(v.grp);v.bodyMat.dispose()}vehs=[]};
 ...
 const topup=(cx:number,cz:number)=>{const pT=mobile?10:PED_MAX,mT=mobile?1:MOTO_MAX,cT=mobile?3:CAR_MAX;if(peds.length<pT)spawnPed(cx,cz);if(vehs.filter(v=>v.type==='moto').length<mT)spawnVeh('moto',cx,cz);if(vehs.filter(v=>v.type==='car').length<cT)spawnVeh('car',cx,cz)};
 let lastTop=0;
 const handle:NpcHandle={
  ...
  update(dt,px,pz){lastTop-=dt;if(lastTop<=0){lastTop=.5;topup(px,pz)}
   ...
   for(const v of vehs){v.life+=dt;const dx=px-v.x,dz=pz-v.z;const dist=Math.hypot(dx,dz);let eff=v.cap;v.yield=false;
    ...
    if(dist>430){v.grp.parent?.remove(v.grp);v.bodyMat.dispose();continue}
    ...
    for(const w of v.wheels)w.rotation.z+=(v.speed*dt)/.36;
    alive.push(v);
   }
   vehs=alive;
   const aliveP:Ped[]=[];
