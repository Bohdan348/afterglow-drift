...
export async function loadCarModelMulti(url:string,flip=false):Promise<CarModel|null>{
  try{
    ...
    const bake=flip?new T.Matrix4().makeRotationY(Math.PI).multiply(new T.Matrix4().makeTranslation(-center.x,-box.min.y,-center.z)):new T.Matrix4().makeTranslation(-center.x,-box.min.y,-center.z);
